...
- `src/styles/tokens.css` — 자동 생성이에요. 고치려면 axhub-template 레포의 `design/tokens.css` 를 고치고 `node scripts/sync-design.mjs` 를 돌려요.
- `src/components/AppShell.tsx` — 앱 전체 뼈대예요. 내용만 그 안에 넣어요.

### 화면·메뉴 추가

- 새 화면은 셸 안에 내용만 넣어요.
- 사이드바 메뉴는 `src/config/navigation.ts` 배열에 한 줄 추가해요.
