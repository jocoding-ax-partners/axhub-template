...
## axhub.ts 신뢰 모델 (모든 템플릿)

각 템플릿의 `axhub.ts` 헬퍼는 6개 템플릿 모두 **동일한 외부 API** 를 노출해요:
`axhub.fetch(path)`, `axhub.data(resource)`, `axhub.slug`, `axhub.isConfigured`.
**다른 템플릿 코드를 복사해도 호환돼요.**

차이는 transport 한 가지뿐 (구조적 차이, 게으름 아님):

| 템플릿 | 위치 | Transport |
|--------|------|-----------|
| nextjs-axhub | `lib/axhub.ts` | `Authorization: Bearer ${process.env.APPHUB_API_KEY}` (Server) |
| express-axhub | `lib/axhub.js` | 동일 (Server) |
| remix-axhub | `app/lib/axhub.server.ts` | 동일 (Server) |
| astro-axhub | `src/lib/axhub.ts` | 동일 (Server) |
| hono-axhub | `src/lib/axhub.ts` | 동일 (Server) |
| **vite-react-axhub** | `src/lib/axhub.ts` | `credentials: "include"` (Browser, **시크릿 키 미주입**) |

**규칙:** `APPHUB_API_KEY` 는 5개 server 템플릿의 `process.env` 에만 존재.
브라우저 (`vite-react-axhub`) 에서는 절대 접근 안 됨 — 빌드 결과물에 시크릿 박히면 누구나 봄.
브라우저 SPA 에서 인증된 axhub 호출이 필요하면, 별도 백엔드 (`express-axhub` 또는 `hono-axhub`) 두고 거기서 처리.
