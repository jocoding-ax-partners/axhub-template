...
|---|---|---|---|
| `vite-react` | `credentials:"include"` (cookie) | `import.meta.env.VITE_APPHUB_*` | **없음** (raw fetch만) |
| `nextjs` | `Authorization: Bearer ${APPHUB_API_KEY}` | `process.env.APPHUB_*` | 없음 (server) |
| `astro` | `Authorization: Bearer ${APPHUB_API_KEY}` | `process.env.APPHUB_*` | 없음 (server) |

ADR-0001 의 **P5 carve-out** 그대로 — "전송(server Bearer vs browser cookie)은 구조적 차이". 즉 **silent SSO(브라우저 redirect)는 vite-react 에만** 의미가 있고, server 템플릿의 Bearer 모델은 별개다 → **Q1 (핵심 미결)**.
