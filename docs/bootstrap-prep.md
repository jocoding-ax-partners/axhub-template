...
> **상태:** Historical draft — 설계 결정 6/6 확정 뒤 템플릿 구현 완료. 2026-06-08 기준 server 템플릿은 `@ax-hub/sdk 3.x`, Vite 는 브라우저 전용 세션 fetch 헬퍼로 정렬됨. 작성: 2026-05-20, ksro0128 (초안: Claude).
...
| `nextjs` | `@ax-hub/sdk 3.x` request-scoped `AxHubClient` (`_hub_access` → SDK JWT) | `process.env.APPHUB_*` + placeholder fallback | 없음 (server) |
| `astro` | `@ax-hub/sdk 3.x` request-scoped `AxHubClient` (`Astro.request` cookie ctx → SDK JWT) | `import.meta.env.APPHUB_*` / `process.env.APPHUB_*` + placeholder fallback | 없음 (server) |

ADR-0001 의 **P5 carve-out** 그대로 — 자격은 `_hub_access` 사용자 세션 JWT 로 통일하지만 전송은 구조적 차이. 즉 **silent SSO(브라우저 redirect)는 vite-react 에만** 의미가 있고, server 템플릿은 SDK 3.x factory 가 들어온 쿠키를 JWT 로 포워딩한다.
...
**nextjs/astro (server) — 구현 완료(Q1): 사용자 세션 JWT 포워딩, 정적 API key 폐기, `@ax-hub/sdk 3.x` 사용.**
