...
5. **Convergence** — **carve-out: 자격(사용자 세션 JWT)은 3종 통일하되 호출 표면은 runtime 별로 분리. Browser SPA 는 `axhub.fetch/data/slug/isConfigured`, server SSR 은 `@ax-hub/sdk 3.x` factory(`makeAxhub`/`makeApp`/`table`)를 쓴다. 전송 방식(browser 자동 쿠키 vs SSR 쿠키 포워딩)은 구조적 차이라 carve-out** (§부트스트랩 인증·설정).
...
- **nextjs / astro (server):** `@ax-hub/sdk 3.x` 의 request-scoped `AxHubClient` 를 만든다. data/identity/gateway 호출은 들어온 요청의 `_hub_access` 를 SDK JWT 로 전달하고 SDK 가 `Authorization: Bearer` 로 처리한다. (Next: `next/headers` `cookies()`, Astro: 호출 시 `Astro.request` 쿠키 ctx 전달.) 모듈 레벨 client 캐싱 금지.
...
- **SDK bundling:** `@ax-hub/sdk 3.x` 는 Node/server 템플릿 전용. Vite 정적 SPA 브라우저 번들에는 넣지 않는다.
