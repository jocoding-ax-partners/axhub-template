...
## 검증 (staging) — 2026-09-08 실증 완료

`axhub up`(로컬 소스 배포, placeholder 를 staging 값으로 치환)으로 두 앱에 올려 확인했다.

| 항목 | nextjs `public-test-9kkc`(로그인 요구 OFF) | vite `app-2rhw`(로그인 요구 ON) |
|---|---|---|
| 익명 접속 | "로그인하지 않았어요" + 로그인 버튼 (오류 문구 없음) | 게이트가 시작점으로 302 |
| 로그인 | 버튼 1클릭 → 시작점 → 콜백 → "환영합니다, 김수로님" + 이메일 + `public (owner)` | 콘솔 로그인된 브라우저는 클릭 0회로 진입 + 이름 표시 (SC-008) |
| `/__axhub/me` | — | 헤더 7종 JSON 반환, `name_b64` 한글 디코드 정상 |
