  ...
  return (
    <main className="relative isolate min-h-screen overflow-hidden bg-[var(--bg-surface)] text-[var(--fg-default)]">
      ...
      <div className="mx-auto flex min-h-screen max-w-xl flex-col items-center justify-center gap-7 px-6 py-20">
        ...
        {/* 데이터 데모 — 브라우저 localStorage 에 저장하는 할 일 (app/todo-demo.tsx) */}
        <section className="w-full rounded-2xl border border-[var(--border-default)] bg-[var(--bg-content)] p-6 shadow-sm">
          <div className="mb-3 flex items-center justify-between">
            ...
            <code className="rounded bg-[var(--primary-soft)] px-1.5 py-0.5 text-[11px] text-[var(--primary)]">localStorage · app/todo-demo.tsx</code>
          </div>
        ...
        <div className="grid w-full grid-cols-1 gap-3 sm:grid-cols-3">
          ...
          <Step n="2" title="데이터 저장" code="app/todo-demo.tsx" />
 ...
 *
 * 1) 데이터 저장·조회 · 표준 PostgreSQL  ★기본 꺼짐 — 쓰려면 먼저 켜야 해요
 *    이 템플릿은 DB 없이 시작해요. 홈 화면 데모(app/todo-demo.tsx)는 브라우저
 *    localStorage 에만 저장해서 기기를 옮기면 사라져요. 서버에 남기려면:
 *
 *      ① axhub.yaml 에 두 줄 추가 (이게 발급 트리거예요)
 *           database:
 *             engine: postgres
 *      ② 로컬은 `npm run db:up` 으로 Postgres 를 띄우고 .env.local 에 DATABASE_URL 을 채워요.
 *         배포하면 axhub 가 전용 DB 를 발급하고 DATABASE_URL / DIRECT_DATABASE_URL 을 자동 주입해요.
 *      ③ 헬퍼는 이미 있어요 (lib/db.ts) — 아래처럼 서버에서 쓰면 돼요.
 *
 ...
 *
 * 4) 에러 처리
 *    - DB(lib/db.ts, 켠 경우) 호출은 표준 postgres 에러를 던져요 — try/catch 로 감싸요.
